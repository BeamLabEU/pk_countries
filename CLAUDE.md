...
- The `:yamerl` application must be started before compilation (handled via `Application.start(:yamerl)`)

### Module Structure

- `PkCountries` (lib/countries.ex) - Main API: `all/0`, `get/1`, `filter_by/2`, `exists?/2`
...
- `PkCountries.Loader` - Compile-time YAML parser, converts yamerl's charlist output to Elixir types
- `PkCountries.Utils` - Helper for converting yamerl tuples/lists to maps
